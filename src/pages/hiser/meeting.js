...
class Meeting extends React.Component{
    ...
    delete = (record)=>{
        axios({
            url : '/bk/index.php/studentsys/deleteMeeting',
            method : 'post',
            data: record
        }).then((res)=>{
            if(res.data.ret){
    ...
    componentDidMount(){
        axios({
            url : '/get/index.php/studentsys/getMeeting'
        }).then((res)=>{
            if(res.data.ret){
    ...
    handlesearch(value){
        ...
        this.newdata = this.data.filter(function(item,index){
            if(item.datetime.indexOf(value)!==-1){
                return true
            }else{
                return false
            }
    ...
    handlesubmit = ()=>{
        if(this.state.type === 'see'){
            ...
            axios({
                url : '/bk/index.php/studentsys/updateMeeting',
                data : newarr,
                method : 'post'
            }).then((res)=>{
                if(res.data.ret){
            ...
            axios({
                url : '/bk/index.php/studentsys/addMeeting',
                data : this.datalist,
                method : 'post'
            }).then((res)=>{
                if(res.data.ret){
                ...
                >
                    <Form onSubmit={this.handlesubmit}>
                        <FormItem
                            ...
                            label='会议类型'
                        >
                        {getFieldDecorator('type')(<Input disabled={sessionStorage.getItem('type')==='s'} placeholder={`请输入活动主题`} />)}
                        </FormItem>
                        <FormItem
                            key={2}
                            {...formItemLayout}

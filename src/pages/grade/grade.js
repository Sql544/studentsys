...
class TeacherMsg extends Component{
    constructor(props){
        ...
        this.prop = {
            name: 'file',
            action: '/bk/index.php/studentsys/excel_grade',
            ...
            onChange(info) {
              if (info.file.status === 'done') {
                  if(info.file.response.ret){
                    ...
                    axios({
                        url : '/get/index.php/studentsys/getstuScore',
                        method : 'get',
                    }).then((res)=>{
                        if(res.data.ret){
    ...
    componentDidMount(){
        if(sessionStorage.getItem('type')==='s'){
            ...
            this.columns = [{
                ...
                dataIndex : 'stuname',
                width : 150,
                sorter: (a, b) => {return a.stuname.localeCompare(b.stuname, 'zh-Hans-CN', {sensitivity: 'accent'})},
                render: (text, record) => this.renderColumns(text, record, 'stuname'),
                ...
                dataIndex : 'term',
                width : 150,
                render: (text, record) => this.renderColumns(text, record, 'term'),
    ...
    handlesearch(value){
        ...
        this.newdata = this.data.filter(function(item,index){
            if(item.stuid.indexOf(value)!==-1||item.stuname.indexOf(value)!==-1){
                return true
            }else{
                return false
            }
    ...
    handleChange(value, key, column) {
      ...
      delete(key){
        ...
        if(target){
            ...
            axios({
                url : '/bk/index.php/studentsys/deletestuScore',
                data : newData[index],
                method : 'post'
            }).then((res)=>{
                if(res.data.ret){
      ...
      save(key) {
        ...
        if (target) {
            ...
            axios({
                url: '/bk/index.php/studentsys/updatestuScore',
                method : 'post',
                data : target
            }).then((res)=>{
                if(res.data.ret){
            ...
            <div>
                ...
                <Col span={4}>
                        <Button type="primary" style={{marginRight:10}}>
                             <a href="http://ozet8lhr5.bkt.clouddn.com/学生绩点模版.xlsx" style={{color:'white'}}>下载模版</a>
                        </Button>
